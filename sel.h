Matrix createLocalK(int element,mesh &m){
    ...
    int e = m.getParameter(E);
    int a = m.getParameter(A);
    float l = m.getNode(element+1).getX() - m.getNode(element).getX();
    int aux = e*a;
    float aux2 = 2*l;
    row1.push_back(aux/aux2); row1.push_back(-aux/aux2);
    row2.push_back(-aux/aux2); row2.push_back(aux/aux2);
